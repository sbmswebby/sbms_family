...
import { DEMO_REGISTRATIONS, DEMO_ACTIVITIES } from "@/components/Activity/demoActivities";
...
export const RegistrationsModal: React.FC<RegistrationsModalProps> = ({
  ...
  return (
    /* 1. BACKDROP: Click listener here closes the modal */
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"
      onClick={onClose}
    >
      {/* 2. MODAL BOX: stopPropagation prevents the click from reaching the backdrop */}
      <div 
            ...
            relevantRegistrations.map((reg) => (
              <div key={reg.id} className="flex items-center justify-between gap-4 border-b border-gray-800 py-3 last:border-0 hover:bg-gray-800/50 px-2 rounded-lg transition-colors">
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <span className="capitalize px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-400 text-[10px] font-bold border border-blue-500/20">
